...
from v9.agent.model import ModelOptimizedQLearningAE

LEARN_RATE = 0.0050
DISCOUNT_FACTOR = 0.10


class AgentOptimizedQLearningAE(AbstractAgent):
    def __init__(self, representation=None):
        ...
        if isinstance(representation, AgentRepresentation):  # build from representation
            ...
            self.model = ModelOptimizedQLearningAE(learn_rate=self.learn_rate, num_configs=self.num_output)
            ...
            self.model = ModelOptimizedQLearningAE(learn_rate=self.learn_rate, num_configs=num_configs)
    ...
    def update_error(self, error, reward, selected_action, curr_q_values, next_q_values, is_done):
        # print("AGENT: R sel c-qval n-qval", reward, selected_action, curr_q_values, next_q_values)
        if is_done:
