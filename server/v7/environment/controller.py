...
class ControllerIdealADSarsa(AbstractController):
    def loop_episodes(self, agent):
        start_timestamp = datetime.now().strftime("%Y-%m-%d--%H-%M-%S")
        run_info = "p{}-{}e-{}s".format(get_prototype(), MAX_EPISODES_V7, SIM_CORPUS_SIZE_V7)
        ...
        eps_iter = range(1, MAX_EPISODES_V7 + 1) if DEBUG_PRINTING else tqdm(range(1, MAX_EPISODES_V7 + 1))
        for episode in eps_iter:
            ...
            while not is_rw_done():
                ...
                if is_simulation() and sim_encryption_progress >= SIM_CORPUS_SIZE_V7:
        ...
        print("steps total", num_total_steps, "avg", num_total_steps / MAX_EPISODES_V7)
        ...
        results_plots_file = plot_average_results(all_summed_rewards, all_avg_rewards, all_num_steps, MAX_EPISODES_V7,
                                                  description)
